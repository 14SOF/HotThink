...
CREATE TABLE IF NOT EXISTS `HotThink`.`comment` (
  ...
  `comment_status` TINYINT NULL,
  `post_post_idx` INT UNSIGNED NOT NULL,
  `user_user_idx` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`comment_idx`, `post_post_idx`, `user_user_idx`),
  INDEX `fk_comment_post1_idx` (`post_post_idx` ASC) VISIBLE,
  INDEX `fk_comment_user1_idx` (`user_user_idx` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`message` (
  ...
  INDEX `fk_message_user1_idx` (`user_receiver` ASC) VISIBLE,
  INDEX `fk_message_user2_idx` (`user_sender` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`report` (
  ...
  INDEX `fk_report_user1_idx` (`user_user_idx` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`follow` (
  ...
  INDEX `fk_follow_user1_idx` (`user_follower` ASC) VISIBLE,
  INDEX `fk_follow_user2_idx` (`user_following` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`point` (
  ...
  INDEX `fk_point_user1_idx` (`user_user_idx` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`rate` (
  ...
  INDEX `fk_rate_user1_idx` (`user_user_idx` ASC) VISIBLE,
...
CREATE TABLE IF NOT EXISTS `HotThink`.`transaction` (
  ...
  INDEX `fk_transaction_post1_idx` (`post_post_idx` ASC) VISIBLE,
  INDEX `fk_transaction_user1_idx` (`user_user_idx` ASC) VISIBLE,
...
-- Table `HotThink`.`like`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS HotThink.like (
  like_idx INT UNSIGNED NOT NULL AUTO_INCREMENT,
  user_user_idx INT UNSIGNED NOT NULL,
  post_post_idx INT UNSIGNED NULL,
  comment_comment_idx INT UNSIGNED NULL,
  PRIMARY KEY (like_idx, user_user_idx),
  INDEX fk_like_user1_idx (user_user_idx ASC),
  INDEX fk_like_post1_idx (post_post_idx ASC),
  INDEX fk_like_comment1_idx (comment_comment_idx ASC),
  ...
  CONSTRAINT fk_like_comment1
    FOREIGN KEY (comment_comment_idx)
    REFERENCES HotThink.comment (comment_idx)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
