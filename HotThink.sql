...
CREATE TABLE IF NOT EXISTS `HotThink`.`message` (
  ...
  UNIQUE INDEX `쪽지번호_UNIQUE` (`message_idx` ASC),
...
-- -----------------------------------------------------
-- Table `HotThink`.`transcation`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `HotThink`.`transcation` (
  `transcation_idx` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `transcation_dateTime` DATETIME NOT NULL,
  `transcation_price` INT UNSIGNED NOT NULL,
  `post_post_idx` INT UNSIGNED NOT NULL,
  `user_user_idx` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`transcation_idx`, `post_post_idx`, `user_user_idx`),
  INDEX `fk_거래내역_post1_idx` (`post_post_idx` ASC),
  INDEX `fk_거래내역_user1_idx` (`user_user_idx` ASC),
  CONSTRAINT `fk_거래내역_post1`
    FOREIGN KEY (`post_post_idx`)
    REFERENCES `HotThink`.`post` (`post_idx`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_거래내역_user1`
    FOREIGN KEY (`user_user_idx`)
    REFERENCES `HotThink`.`user` (`user_idx`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
